...
export const handlers = [
  ...
  http.post(`${API_DOMAIN}/api/v1/urls`, async ({ request }) => {
    const { slug, targetUrl } = (await request.json()) as { targetUrl: string; slug: string };
    const newUrl = {
  ...
  http.get(`${API_DOMAIN}/api/v1/urls`, ({ request }) => {
    ...
    switch (sort) {
      case 'oldest':
        sortedUrls.sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        break;
      case 'most-clicks':
      ...
      case 'newest':
      default:
        sortedUrls.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        break;
    }
  ...
  // Update URL
  http.patch(`${API_DOMAIN}/api/v1/urls/:urlId`, async ({ params, request }) => {
    const { urlId } = params;
    const { slug, targetUrl } = (await request.json()) as { targetUrl?: string; slug?: string };
    ...
    return HttpResponse.json({
      urlId: Number(urlId),
      shortenedUrl: `https://scg.io/${slug || mockUrls[index]?.slug}`,
