...
import { ArrowLeft, ChartBar, PencilSimple, Trash, Link as LinkIcon } from "@phosphor-icons/react";
import TextReveal from "@/app/components/ui/TextReveal";
import UrlForm from "@/app/components/ui/UrlForm";
...
export default function DashboardPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [userData, linksResponse] = await Promise.all([
        ...
        api.get<{ data: LinkData[], meta: { total: number; page: number; limit: number; totalPages: number } }>(`/api/v1/urls?page=${page}&limit=5&sort=${sort}`),
      ]);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await api.patch(`/api/v1/urls/${editingLink.urlId}`, { slug: newSlug, targetUrl: newTargetUrl });
      await fetchData();
      setEditingLink(null);
    } catch (error) {
    ...
    <ReactLenis root>
      <div className="min-h-screen bg-soft-gray p-8 selection:bg-foreground selection:text-background">
        <nav className="mb-20 flex items-center justify-between">
          <Link href="/" className="group flex items-center gap-2 text-sm font-medium text-zinc-500 hover:text-foreground">
        ...
        <main className="mx-auto max-w-6xl">
          <div className="mb-24">
            ...
            <p className="mt-6 max-w-md text-xl text-zinc-500">
              Manage your links and access analytics. 
              Keep it simple.
            </p>
          </div>
          ...
          <div className="grid gap-16 lg:grid-cols-[1fr,1.5fr]">
            <div className="space-y-12">
              <div className="rounded-3xl bg-background p-8 shadow-sm">
                <h2 className="mb-8 text-2xl font-bold tracking-tight">Create New</h2>
            ...
            <div className="space-y-8">
              ...
              {isLoading ? (
                ...
                <div className="py-12 text-center text-zinc-400">No links found.</div>
              ) : (
                  ...
                  {links.map((link) => (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <h3 className="mb-1 font-display text-2xl font-bold">/{link.slug}</h3>
                          <p className="text-sm text-zinc-400 truncate max-w-xs">{link.targetUrl}</p>
                        </div>
                        <div className="text-right">
                          <span className="block text-3xl font-bold">{link.totalClicks}</span>
                          <span className="text-xs uppercase tracking-wide text-zinc-400">Clicks</span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 border-t border-zinc-100 overflow-hidden transition-all duration-300 ease-out max-h-0 opacity-0 mt-0 pt-0 group-hover:max-h-20 group-hover:mt-6 group-hover:pt-4 group-hover:opacity-100 dark:border-zinc-700">
                        ...
                        <div className="ml-auto flex gap-2" onClick={(e) => e.stopPropagation()}>
                           <button onClick={() => router.push(`/analytics/${link.slug}`)} className="rounded p-2 hover:bg-zinc-100 dark:hover:bg-zinc-700" title="Analytics">
                             <ChartBar weight="bold" />
                           </button>
                           <button onClick={() => handleEditClick(link)} className="rounded p-2 hover:bg-zinc-100 dark:hover:bg-zinc-700" title="Edit">
                             <PencilSimple weight="bold" />
                           </button>
                           <button onClick={() => setDeletingLink(link)} className="rounded p-2 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20" title="Delete">
                             <Trash weight="bold" />
                           </button>
                        </div>
                      </div>
